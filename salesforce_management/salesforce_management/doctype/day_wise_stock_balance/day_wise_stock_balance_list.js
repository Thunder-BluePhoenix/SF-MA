frappe.views.ListView = class ListView extends frappe.views.ListView {
    ...
    get_list_row_html_skeleton(left = "", right = "", employee_name = "", store="", warehouse_balance="", manual_balance_entry="", mismatched_qty="") {
        ...
        if (this.doctype === "Day Wise Stock Balance") {
    		return `
    			<div class="list-row-container" tabindex="1">
                    ...
                    <div class="level list-row details-row">Store value: ${warehouse_balance}</div>
					<div class="level list-row details-row">Manual Entry: ${manual_balance_entry}</div>
                    <div class="level list-row details-row">Mismatched Qty: ${mismatched_qty}</div>
    			</div>
    		`;
        }else{
			return `
	...
	get_list_row_html(doc) {
		return this.get_list_row_html_skeleton(
			...
			doc.employee_name,
			doc.store,
            doc.warehouse_balance,
            doc.manual_balance_entry,
            doc.mismatched_qty
		);
	}
}
