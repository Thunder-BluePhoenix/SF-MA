...
class StockBalanceReport(object):
	...
	def prepare_new_data(self):
		...
		for key, report_data in self.item_warehouse_map.items():
			...
			min_qty = frappe.db.get_value("Item Reorder", {"parent": report_data.get("item_code"), "warehouse": report_data.get("warehouse")}, 'warehouse_reorder_level')
			report_data["min_quantity"] = min_qty if min_qty else 0.00
			
			report_data["difference_quantity"] = report_data.get("bal_qty") - (min_qty if min_qty else 0.00)
			frappe.logger("utils").exception(report_data.get("warehouse"))
			if str(report_data.get("warehouse")) in warehouses:
				store = frappe.db.get_value("Warehouse", report_data.get("warehouse"), "store")
				store_details = frappe.db.get_value("Store", store, ["store_name", "zone"], as_dict=True)
				if store_details:
					report_data['zone'] = store_details.get("zone")
					report_data['store_name'] = store_details.get("store_name")
	...
	def get_columns(self):
		...
		if designation == "Promoter":
			return get_columns_for_promoter()
		columns = [
			{
				"label": _("Item"),
				"fieldname": "item_code",
		...
		if self.filters.get("show_stock_ageing_data"):
			columns += [
				{"label": _("Average Age"), "fieldname": "average_age", "width": 150},
				{"label": _("Earliest Age"), "fieldname": "earliest_age", "width": 150},
				{"label": _("Latest Age"), "fieldname": "latest_age", "width": 150},
			]

		if self.filters.get("show_variant_attributes"):
			columns += [
				{"label": att_name, "fieldname": att_name, "width": 150}
...
def get_warehouse_of_user(employee):
	try:
		shift = frappe.db.get_list('Shift Assignment', {'employee': employee, "docstatus": ("!=", 2)}, ['store','name','start_time', 'end_time', "floater"])
		if shift:
			if shift[0].get("floater"):
				floater_store = frappe.db.get_all("Floater Store", {"parent": shift[0].get("name")}, "store")
				floater_stores = [f"{i.get('store')} - SB" for i in floater_store if i.get("store")]
				primary_store = frappe.db.get_value("Warehouse", {'store': shift[0].get("store")}, 'name')
				floater_stores.append(primary_store)
				return floater_stores
			else: 
				frappe.logger("utils").exception("This")
				return frappe.db.get_value("Warehouse", {'store': shift[0].get("store")}, 'name')
		return None
	except Exception as e:
		frappe.logger("utils").exception(e)
		return None

def get_columns_for_promoter():
	columns = [
			{
				"label": _("Item"),
				"fieldname": "item_code",
...
def get_warehouses():
	if frappe.session.user == 'Administrator':
		return [i.get("name") for i in frappe.db.get_all("Warehouse", {}, 'name')]
	emp_doc = frappe.get_doc("Employee", {"user_id": frappe.session.user})
	names = []
	
	if emp_doc.designation == "Promoter":
		names.append(emp_doc.name)
	elif emp_doc.designation == "Team Leader":
		promoter_list = frappe.db.get_list("Employee", {}, ["name"])
		names.extend(i.get("name") for i in promoter_list if i.get("name"))

	elif emp_doc.designation == "Supervisor":
		names.extend(
			i.get("name")
		...
		tls = frappe.db.get_all("Employee", {"reports_to": emp_doc.name, "designation": "Team Leader"}, ["name"])
		for tl in tls:
			promoter_list = frappe.db.get_all("Employee", {"reports_to": tl.get("name"), "designation": "Promoter"}, ["name"])
			names.extend(i.get("name") for i in promoter_list if i.get("name"))
			
	elif emp_doc.designation == "ASM":
		names.extend(
			i.get("name")
			for i in frappe.db.get_list(
				"Employee", {}, ["name"]
		...
		# frappe.throw(emp_doc.name)
		sos = frappe.db.get_all("Employee", {"reports_to": emp_doc.name, "designation": "Supervisor"}, ["name"])
		for so in sos:
			tls_list = frappe.db.get_all("Employee", {"reports_to": so.get("name")}, ["name"])
			for tl in tls_list:
				promoter_list = frappe.db.get_all("Employee", {"reports_to": tl.get("name")}, ["name"])
				names.extend(i.get("name") for i in promoter_list if i.get("name"))
	
	warehouses = []
	for emp in names:
		warehouse = get_warehouse_of_user(emp)
		if warehouse:
			if isinstance(warehouse, list):
				warehouses += warehouse
			else:
				warehouses.append(warehouse)
	frappe.logger("utils").exception(warehouses)
	return warehouses
