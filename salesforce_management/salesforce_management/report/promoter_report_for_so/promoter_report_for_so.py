...
def get_columns():
    return [
        ...
        {"label": "Order ID", "fieldname": "order_id", "fieldtype": "Link", "options": "Sales Order", "width": 150},
        ...
        {"label": "Promoter Code", "fieldname": "promoter_code", "fieldtype": "Data", "width": 120},
        {"label": "Promoter Name", "fieldname": "promoter_name", "fieldtype": "Data", "width": 180},
...
def get_data(filters):
    ...
    SELECT 
        ...
        sa.employee as promoter_code,
        sa.zone as zone,
        store.custom_city_name as city,
        employee.employee_name as promoter_name,
        so.total_qty
    FROM 
        `tabSales Order` so
    LEFT JOIN 
    ...
    LEFT JOIN 
        `tabShift Assignment` sa ON warehouse.store_name = sa.store_name
    LEFT JOIN 
        `tabEmployee` employee ON sa.employee = employee.name
    WHERE 
        so.docstatus = 1 
        AND {condition_query}
    ORDER BY 
