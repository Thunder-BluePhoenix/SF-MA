frappe.pages['mark-attendance'].on_page_load = async function (wrapper) {
    ...
    const response = await frappe.call({
        method: 'salesforce_management.salesforce_management.page.mark_attendance.mark_attendance.validate_attendance'
    });
    ...
    const floater_response = await frappe.call({
        method: 'salesforce_management.salesforce_management.page.mark_attendance.mark_attendance.get_employee_details'
    });
    ...
    if (!floater_response.exc) {
        ...
        if (floater_data) {
            ...
            if (floater_data.is_floater != 1) {
                ...
                const location = await resolveWithTimeout(
                    window?.nativeInterface?.execute('getLocation').catch((err) => {
                        console.log(err);
                        frappe.throw(err);
                    }),
                    10000
                ...
                if (latitude && longitude) {
                    const isUserOnLocation = await frappe.call({
                        method: 'salesforce_management.salesforce_management.page.mark_attendance.mark_attendance.validate_location',
                        args: {
            ...
            } else if (floater_data.is_floater == 1) {
                // Get Geo Fence Settings using frappe.client.get method
                const geoFenceSettings = await frappe.call({
                    method: 'frappe.client.get',
                    args: {
                        doctype: 'Geo Fence Settings'
                    }
                });
                
                if (geoFenceSettings.message.track_geo_floater === 'Yes') {
                    const Loader = $(`<div style="width: 100%;">
                        <p class="pulse text-center" style="margin: auto;">
                            Please wait while we are trying to get your location
                        </p>
  ...
  <div class="modal-dialog">
    ...
    const check_face_detected = async ({name}) => {
        ...
        return new Promise((resolve, reject) => {
            const interval = setInterval(() => {
                frappe.call({
                    method: 'salesforce_management.salesforce_management.page.mark_attendance.mark_attendance.validate_face',
                    args: {
                        name
                    },
                    callback: (r) => {
    ...
    checkInButton.click(async function () {
        ...
        const checkinResponse = await frappe.call({
            method: 'salesforce_management.salesforce_management.page.mark_attendance.mark_attendance.create_employee_checkin',
            args: {
    ...
    checkOutButton.click(async function () {
        ...
        const attendanceResponse = await frappe.call({
            method: 'salesforce_management.salesforce_management.page.mark_attendance.mark_attendance.mark_attendance',
            args: {
