frappe.pages['stock-taking'].on_page_load = async function (wrapper) {
	...
	// Add the fields
    var employeeField = addField('employee', 'Employee', 'Link', "Employee");
    var employeeNameField = addField('employee_name', 'Employee Name', 'Data', "");
    var storeField = addField('store', 'Store', 'Link', "Store");
    var itemField = addField('item', 'Item', 'Link', "Item");
	var openBarcodeField = addField('scan', 'Scan', 'Button', "");
	var qtyField = addField('qty', 'Manual Quantity', 'Int', "");
    ...
    `)
        .appendTo(mainDiv);
    const openStockSuccessModalBtn = $(`<button id="openStockSuccessModal" type="button" class="hidden" data-toggle="modal" data-target="#StockSuccessModal"></button>`).appendTo(mainDiv);
	...
	var submitButton = page.set_primary_action(__('Submit'), function() {
        ...
        frappe.call({
            method: 'salesforce_management.salesforce_management.page.stock_taking.stock_taking.create_stock_balance',
            args: {
                store: store_field,
            ...
            callback: function(response) {
                if (response.message) {
                    openStockSuccessModalBtn.click();
