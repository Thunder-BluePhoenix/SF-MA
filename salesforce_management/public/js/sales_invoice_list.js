frappe.views.ListView = class ListView extends frappe.views.ListView {
    ...
    get_list_row_html_skeleton(left = "", right = "", store = "", date="", quantity="", total="") {
        ...
        if (this.doctype === "Sales Invoice") {
    		...
    		return `
    			<div class="list-row-container" tabindex="1">
    				...
    				<div class="level list-row details-row">Start Date: ${date}</div>
    				<div class="level list-row details-row">Total Quantity: ${quantity}</div>
					<div class="level list-row details-row">Total: ${total}</div>
    			</div>
    		`;
        }else{
            return `
